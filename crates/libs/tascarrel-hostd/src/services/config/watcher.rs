//! Native filesystem event delivery for the configuration service.
//!
//! [`WatchEvents`] owns the platform-recommended recursive watcher and adapts
//! its callback into a bounded asynchronous event stream.
...
impl WatchEvents {
    /// Opens the platform-recommended recursive watcher for `root`.
        ...
        let mut watcher = notify::recommended_watcher(move |result: notify::Result<Event>| {
            ...
            match sender.try_send(message) {
                ...
                Err(TrySendError::Closed(_)) => {
                    debug!("configuration watcher event channel is closed");
                }
...
pub(crate) enum WatcherError {
    ...
    #[error("failed to initialize configuration filesystem watcher")]
    Initialize,
